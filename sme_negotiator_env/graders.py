...
def _strict_unit_interval(score: float) -> float:
    """Map any numeric score into the strict open interval (0, 1)."""
    value = float(score)
...
def compute_financing_npv_vs_status_quo(state: "NegotiationState") -> float:
    ...
    Uses `interest_rate_annual` as the annual discount rate.
    """
...
def grade_task_payment_terms_easy(state: "NegotiationState") -> float:
    """Easy task:
    Success means agreed payment days are at or below the liquidity threshold.
    """
...
def grade_task_payment_terms_medium(state: "NegotiationState") -> float:
    """Medium task:
    Primary success = agreed days at or below liquidity threshold.
    Secondary partial credit = slightly above threshold, but with late-payment penalty.
    """
    if not state.deal_reached or state.agreed_terms is None:
        return _strict_unit_interval(0.0)
...
def grade_task_dynamic_discounting_hard(state: "NegotiationState") -> float:
    """Hard task:
    Score based on NPV improvement over status quo under dynamic discounting.
    """
